...
iv = [0x00] * 8  # 64-bit IV
with open('imagetest1.jpg', 'rb') as file:
    image_data = file.read()
ciphertext = rabbit_encrypt(image_data, key, iv)
write_ciphertext_to_file(ciphertext, 'Encrypted_bytes.txt')
decrypted_text = rabbit_decrypt(ciphertext, key, iv)
write_ciphertext_to_file(decrypted_text, 'Decrypted_bytes.txt')
with open('decrypted_image2.', 'wb') as file:
